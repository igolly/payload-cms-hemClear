...
export const MedicalReviewBlock: React.FC<Props> = ({
  ...
  return (
    <section
      className="w-full bg-shell px-4 py-14 sm:px-6 lg:px-8"
    ...
    >
      <div className="mx-auto max-w-6xl">
        ...
        {points.length > 0 && (
          <ul className="mx-auto mt-8 grid max-w-4xl grid-cols-1 gap-6 rounded-xl border border-tint-100 bg-white p-6 sm:grid-cols-3">
            {points.map((point, i) => (
              <li
                ...
                key={point.id ?? i}
              >
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-mist-100 text-brand [&>span>svg]:h-5 [&>span>svg]:w-5">
        ...
        {reviewers.length > 0 && (
          <ul className="mt-8 grid grid-cols-1 gap-5 md:grid-cols-3">
            {reviewers.map((doctor, i) => (
              <li
                className="flex flex-col rounded-xl border border-tint-100 bg-white p-5"
                data-payload-subpath={`doctors.${i}.name`}
                key={doctor.id ?? i}
              >
                <div className="flex items-center gap-3">
                  <span className="relative h-12 w-12 shrink-0 overflow-hidden rounded-full bg-mist-100">
                  ...
                  {doctor.tag && (
                    <span className="shrink-0 rounded-full bg-mist-100 px-2.5 py-1 text-[10px] font-semibold text-brand">
                      {marks(doctor.tag)}
                    </span>
                  )}
                </div>
                ...
                {doctor.readMoreLabel && (
                  <p className="mt-3 flex items-center gap-1 text-xs font-bold text-brand-500">
                ...
                {Array.isArray(doctor.tags) && doctor.tags.length > 0 && (
                  <ul className="mt-3 flex flex-wrap gap-1.5">
                    {doctor.tags.map((tag, t) => (
                      <li
                        className="rounded bg-mist-100 px-2 py-1 text-[10px] font-medium text-brand"
                        key={tag.id ?? t}
                      >
                        {marks(tag.text)}
                      </li>
