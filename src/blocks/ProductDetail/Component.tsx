    ...
    <section className="w-full bg-white px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 lg:grid-cols-2">
        ...
        <div>
          ...
          {product.description && (
            <p className="mt-3 text-sm leading-relaxed text-[#1a2f7c]">{marks(product.description)}</p>
          )}
                ...
                {feel.map((row, i) => (
                  ...
                  >
                    ...
                    <span className="min-w-0 grow">
                      <span className="block text-sm font-bold text-brand">{marks(row.title)}</span>
                      {row.subtitle && (
                        <span className="block text-[11px] text-[#0052cc]">{marks(row.subtitle)}</span>
                      )}
                    </span>
                    {row.percent && (
          ...
          {stories.length > 0 && (
            <div className="border-t border-[#dbe8fa] py-6">
              {product.storiesTitle && (
                <h2 className="font-serif text-xl text-heading">{marks(product.storiesTitle)}</h2>
              )}
              <div className="mt-4">
